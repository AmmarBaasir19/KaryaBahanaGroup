        ...
        try : 
            if value1 or value2 : 
                ...
                if ComponentsButton().button_download(): 
                    ...
                    st.write(f"Hai ini data df_polygon")
                    st.write(SessionManager().get_setter_state(key='error_page'))
                    for i in SessionManager().get_setter_state(key='error_page'):
                        st.write(f"Hai ini index error page : {i}")
                ...
                if SessionManager().get_setter_state(key='error_page'): 
                    df_polygon = SessionManager().get_setter_state(key='dataframe_polygon')
                    #df_polygon = df_polygon.drop(index=(SessionManager().get_setter_state(key='error_page') - ))
