            ...
            if value1 or value2 : 
                ...
                if ComponentsButton().button_download(): 
                    ...
                    if SessionManager().get_setter_state(key='error_page') in SessionManager().get_setter_state(key='dataframe_polygon').index.tolist(): 
                        st.write(f"Hai ini ada index yang akan di drop")

                    ###
                    #st.dataframe(SessionManager().get_setter_state(key='dataframe_polygon').reset_index().loc[5])
                    df_polygon_exe = SessionManager().get_setter_state(key='dataframe_polygon').reset_index()
                    if df_polygon_exe is not None and not df_polygon_exe.empty: 
                        st.write(f"Hai ini dataframe polygon sebelum drop error page")
