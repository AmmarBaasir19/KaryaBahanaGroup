        ...
        try : 
            if value1 or value2 : 
                ...
                if ComponentsButton().button_update_all():
                    ...
                    #st.write(f"Ini dataframe_polygon : {len(SessionManager().get_setter_state(key='dataframe_polygon'))}")
                    #st.write(f"Ini processing_page : {len(SessionManager().get_setter_state(key='processing_page'))}") 
                    #st.write(f"Ini start_processing : {(SessionManager().get_setter_state(key='start_processing'))}")
                    #st.write(f"Ini sum_processing : {(SessionManager().get_setter_state(key='sum_processing'))}")
                    #st.write(f"Ini dataframe_noninline before update : {len(SessionManager().get_setter_state(key='dataframe_noninline'))}")
                    ...
                    st.write(f"Dataframe noninline : {SessionManager().get_setter_state(key='dataframe_noninline')}")
