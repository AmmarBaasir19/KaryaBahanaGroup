...
class YieldCalculation: 
    ...
    def yield_generations(self, df: pd.DataFrame) -> pd.DataFrame: 
        ...
        try : 
            df['Final Yield / Line'] = (((df['Qty Checked'] - df['Final Def'])/df['Qty Checked'].replace(0, pd.NA)) * 100).astype(float)
            df['First Pass Yield / Line'] = (((df['Qty Checked'] - df['First Pass Def'])/df['Qty Checked'].replace(0, pd.NA)) * 100).astype(float)
    ...
    def run(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try : 
            ...
            print(f"Ini setelah groupby_data : {df}")
            df = self.yield_generations(df) 
            print(f"Ini setelah yield_generations : {df}")
            df = self.melt_data(df)
            print(f"Ini setelah melt_data : {df}") 
