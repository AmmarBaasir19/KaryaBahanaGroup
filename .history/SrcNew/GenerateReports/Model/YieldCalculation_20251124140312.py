...
class YieldCalculation: 
    ...
    def yield_generations(self, df: pd.DataFrame) -> pd.DataFrame: 
        ...
        try : 
            df['Final Yield / Line'] = round(((df['Qty Checked'].astype(int) - df['Final Def'].astype(int))/df['Qty Checked'].astype(int).replace(0, pd.NA)) * 100, 2)
            df['First Pass Yield / Line'] = round(((df['Qty Checked'] - df['First Pass Def'])/df['Qty Checked'].replace(0, pd.NA)) * 100, 2)
            df['GAP (FY - FPY)'] = round(df['Final Yield / Line'] - df['First Pass Yield / Line'], 2)

            for col in ['Final Yield / Line', 'First Pass Yield / Line', 'GAP (FY - FPY)']:
                df[col] = df[col].round(2).astype(str) + "%"
            
            df['Final Yield / Line'] = float(df['Final Yield / Line'].str.replace('%', '', regex=False)).astype(float)
