        ...
        try : 
            if value1:
                ...
                if ComponentsButton().button_download(): 
                    ...
                    st.write("Ini After Lower Case")
                    st.write(SessionManager().get_setter_state(key='dataframe_fpd'))
                    st.write(df_temp.columns)
                    st.write(df_temp_user.columns) 
                    ...
                    db_engine = DatabaseConfig().connect_to_db()
                    SessionManager().get_setter_state(key='dataframe_fpd').to_sql('fpd', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_fpd()) 
                    SessionManager().get_setter_state(key='dataframe_users').to_sql('users_activity', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_users_activity()) 
