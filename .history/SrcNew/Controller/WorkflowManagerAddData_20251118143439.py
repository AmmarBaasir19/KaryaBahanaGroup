...
class WorkflowManagerAddData: 
    ...
    def __handling_update_fd(self, value1, value2, df1, df2, type_form, uploaded_file):
        ...
        try : 
            if value1 or value2 : 
                ...
                if ComponentsButton().button_update_all():
                    SessionManager().set_setter_state(key='dataset_name', value=f"FRM-QC-026_rev3_{datetime.now().strftime('%Y-%m-%d')}_Final_Defects_Merge_File.pdf")
                    SessionManager().set_setter_state(key='dataframe_inline', value=df1)
                    SessionManager().set_setter_state(key='dataframe_noninline', value=df2)
                ...
                elif ComponentsButton().button_download(): 
                    now_end = datetime.now()
                    SessionManager().generate_dataframe('dataframe_users', 1, 6, "User_Name", "Name", "Nik_Nipm", "Location", "Role", "Login_Id") 
                    ...
                    st.write(f"Hai ini data sebelum join") 
                    st.dataframe(SessionManager().get_setter_state(key='dataframe_polygon')) 
                    st.write(f"Hai ini data sebelum join noninline")
                    st.dataframe(SessionManager().get_setter_state(key='dataframe_noninline'))
                    st.write(f"Hai ini error page: {SessionManager().get_setter_state(key='error_page')}")
                    st.write(f"Hai ini index yang akan di drop: {SessionManager().get_setter_state(key='dataframe_polygon').index.tolist()}")
                    df_polygon_exe = SessionManager().get_setter_state(key='dataframe_polygon')
                    if SessionManager().get_setter_state(key='error_page') in SessionManager().get_setter_state(key='dataframe_polygon').index.tolist(): 
                        st.write(f"Hai ini ada index yang akan di drop if condition") 
                        for i in SessionManager().get_setter_state(key='error_page'):
                            st.write(f"Dropping index: {i - 1}") 
                            df_polygon_exe = df_polygon_exe.drop(index=(i - 1))
                            processing_page = SessionManager().get_setter_state(key='processing_page')
                            processing_page = processing_page.pop(i - 1) 
                        
                        SessionManager().set_setter_state(key='dataframe_polygon', value=df_polygon_exe) 
                        SessionManager().set_setter_state(key='processing_page', value=processing_page)

                    st.write(f"Hai ini data sebelum create data file name") 
                    st.dataframe(SessionManager().get_setter_state(key='dataframe_polygon')) 
                    st.write(len(SessionManager().get_setter_state(key='dataframe_polygon'))) 


                    ## Create Data File Name
                    data = SessionManager().get_setter_state(key='dataframe_noninline')
                    st.dataframe(data)
                    st.write(SessionManager().get_setter_state(key='login_id'))
                    ...
                    st.write(f"3")
                    st.write(f"Ini waktu processing page : {SessionManager().get_setter_state(key='processing_page')}")
                    st.write(len(SessionManager().get_setter_state(key='processing_page')))
                    ...
                    ## Join With Dataset  
                    st.dataframe(data)
                    st.write(f"Hai ini sebelum join noninline dengan polygon") 
                    st.dataframe(SessionManager().get_setter_state(key='dataframe_polygon')) 

                    #df_polygon = SessionManager().get_setter_state(key='dataframe_polygon')
                    #df_polygon = df_polygon.drop(columns=['level_0', 'index'])
                    #df_new = [
                    #    {"table_1": "[83, 133, 1368, 133, 1368, 248, 83, 248]",
                    #     "table_2" : "[82, 266, 1371, 265, 1370, 373, 81, 373]",
                    #     "table_3" : "[1421, 132, 2285, 132, 2285, 367, 1421, 366]",
                    #     "table_4" : "[80, 387, 1157, 386, 1158, 2797, 80, 2798]",
                    #     "table_5" : "[1198, 385, 2285, 384, 2285, 2799, 1199, 2799]",
                    #     "table_6" : "[82, 2853, 2291, 2850, 2291, 3003, 82, 3006]"
                    #     },
                    #     {"table_1": "[83, 133, 1368, 133, 1368, 248, 83, 248]",
                    #     "table_2" : "[82, 266, 1371, 265, 1370, 373, 81, 373]",
                    #     "table_3" : "[1421, 132, 2285, 132, 2285, 367, 1421, 366]",
                    ...
                    #     }
                    #    ]
                    #df_new_pol = pd.concat([df_polygon, pd.DataFrame(df_new)], axis=1)
                    #SessionManager().set_setter_state(key='dataframe_polygon', value=df_new_pol)
                    ...
                    df1_temp = SessionManager().get_setter_state('dataframe_inline')
                    df2_temp = SessionManager().get_setter_state('dataframe_noninline')
                    df_users_temp = SessionManager().get_setter_state('dataframe_users')
                    df1_temp.columns = df1_temp.columns.str.lower()
                    df2_temp.columns = df2_temp.columns.str.lower() 
                    df_users_temp.columns = df_users_temp.columns.str.lower()

                    df1_temp = df1_temp.rename(columns={'nik_nipm': 'nik_nipm',
                                                      'part no': 'part_no',
                                                      'part name': 'part_name',
                                                      'ttd kasie': 'ttd_kasie',
                                                      'ttd operator': 'ttd_operator',
                                                      'tidak tepat': 'tidak_tepat',
                                                      'welding check points': 'welding_check_points'
                                                      })
                    df2_temp = df2_temp.rename(columns={'nik_nipm': 'nik_nipm',
                                                      'part no': 'part_no',
                                                      'part name': 'part_name',
                                                      'ttd kasie': 'ttd_kasie',
                                                      'ttd operator': 'ttd_operator',
                                                      'tidak tepat': 'tidak_tepat',
                                                      'folding keras': 'folding_keras',
                                                      'cat/coating': 'cat_coating',
                                                      'tidak masuk gonogo': 'tidak_masuk_gonogo',
                                                      'step loss': 'step_loss',
                                                      'step loncat': 'step_loncat',
                                                      'step lebih': 'step_lebih',
                                                      'step kurang': 'step_kurang',
                                                      'salah pasang': 'salah_pasang',
                                                      'tidak lengkap': 'tidak_lengkap',
                                                      'catatan problem': 'catatan'
                                                      })
                    
                    df_users_temp = df_users_temp.rename(columns={'user_name': 'username',
                                                                  'name': 'name',
                                                                  'nik': 'nik_nipm',
                                                                  'location': 'location',
                                                                  'role': 'role',
                    ...
                    db_engine = DatabaseConfig().connect_to_db()
                    df2_temp.to_sql('fd_noninline', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_fd_noninline()) 
                    df1_temp.to_sql('fd_inline', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_fd_inline()) 
                    df_users_temp.to_sql('users_activity', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_users_activity()) 
                ...
                elif SessionManager().get_setter_state(key='error_page'): 
                    try :
                        DataProblemScanning().run(input_pdf=SessionManager().get_setter_state(key='pdf_input'), error_page=SessionManager().get_setter_state(key='error_page'), file_name=FileManager().get_format_name(SessionManager().get_setter_state(key='dataframe_noninline')), type_form=type_form) 
                    except Exception as e : 
                        st.error(f"Error in class WorkflowManagerAddData (__handling_update_fd) Condition Error Page : {e}") 
        ...
        try : 
            if value1:
                ...
                if ComponentsButton().button_download(): 
                    st.write(SessionManager().get_setter_state(key='dataframe_fpd'))
                    now_end = datetime.now()
                    st.write(now_end)
                    st.write(SessionManager().get_setter_state(key='start_processing'))
                    SessionManager().generate_dataframe('dataframe_users', 1, 6, "User_Name", "Name", "Nik_Nipm", "Location", "Role", "Login_Id")  
                    ...
                    ## Lower Case Column Names
                    df_temp = SessionManager().get_setter_state(key='dataframe_fpd')
                    df_users_temp = SessionManager().get_setter_state(key='dataframe_users')
                    df_temp.columns = df_temp.columns.str.lower()
                    df_users_temp.columns = df_users_temp.columns.str.lower()

                    df_temp = df_temp.rename(columns={'nik_nipm': 'nik_nipm',
                                                      'part no': 'part_no',
                                                      'part name': 'part_name',
                                                      'part group': 'part_group',
                                                      'total repair': 'total_repair',
                                                      'total top' : 'total_top',
                                                      'total middle' : 'total_middle',
                                                      'total bottom' : 'total_bottom'
                                                      })
                    ...
                    db_engine = DatabaseConfig().connect_to_db()
                    df_temp.to_sql('fpd', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_fpd()) 
                    df_users_temp.to_sql('users_activity', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_users_activity()) 
                ...
                if SessionManager().get_setter_state(key='error_page'): 
                    DataProblemScanning().run(input_pdf=SessionManager().get_setter_state(key='pdf_input'), error_page=SessionManager().get_setter_state(key='error_page'), file_name=FileManager().get_format_name(SessionManager().get_setter_state(key='dataframe_fpd')), type_form=type_form) 
    ...
    def handling_generate_data(self, inp_form, inp_model, inp_no, inp_date, inp_shift):
        ...
        df_1 = pd.DataFrame({
                    ...
                    'part_no': [inp_no] * int(check_points), 
                    'part_name': [part_name] * int(check_points),
                    'ttd_kasie': [' '] * int(check_points),
                    'ttd_operator': [' '] * int(check_points),
                    'nik_nipm': [' '] * int(check_points), 
        ...
        try : 
            ...
            if inp_form == "Final Defects" :
                ...
                if ComponentsButton().button_download():
                    ...
                    ## Save Data
                    dff_1.to_sql('fd_inline', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_fd_inline())
                    dff_2.to_sql('fd_noninline', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_fd_noninline())
                ...
                if ComponentsButton().button_download():
                    ...
                    ## Save Data 
                    dff_3.to_sql('fpd', con=db_engine, if_exists='append', index=False, dtype=DatabaseConfig().mapping_fpd())
