...
class YieldCalculation: 
    def groupby_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try : 
            return (df.groupby(['Model', 'Part Name', 'Bulan', 'Detail_Date'])[
                ['Qty Checked', 'First Pass Def', 'Final Def']].sum().reset_index()
            )
    ...
    def yield_generations(self, df: pd.DataFrame) -> pd.DataFrame: 
        ...
        try : 
            df['Final Yield / Line'] = ((df['Qty Checked'] - df['Final Def'])/df['Qty Checked'].replace(0, pd.NA)) * 100
            df['First Pass Yield / Line'] = ((df['Qty Checked'] - df['First Pass Def'])/df['Qty Checked'].replace(0, pd.NA)) * 100
