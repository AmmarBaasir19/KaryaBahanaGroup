...
class MainAutomaticReports:
    ...
    def run(self, df1: pd.DataFrame, df2: pd.DataFrame, option: str, start_date: pd.to_datetime, end_date: pd.to_datetime) -> pd.DataFrame:
        try : 
            ...
            df_ctrl['Qty Checked'] = df_ctrl['Qty Checked'].fillna(0)
            df_ctrl['First Pass Def'] = df_ctrl['First Pass Def'].fillna(0)
            df_ctrl['Final Def'] = df_ctrl['Final Def'].fillna(0)
            st.dataframe(df_ctrl) 
            print(f"Ini After df_ctrl : {df_ctrl.info()}")

            df_ctrl['Qty Checked'] = df_ctrl['Qty Checked'].astype(int)
            df_ctrl['First Pass Def'] = df_ctrl['First Pass Def'].astype(int)
            df_ctrl['Final Def'] = df_ctrl['Final Def'].astype(int)

            df_yield = YieldCalculation().yield_generations(df_ctrl)
            st.write(f"Ini After df_yield yield_generations : ")
            st.dataframe(df_yield)
            print(f"Ini After df_yield yield_generations : {df_yield.info()}")

            df_yield = YieldCalculation().melt_data(df_yield)
            st.write(f"Ini After df_yield melt_data : ")
            st.dataframe(df_yield)
