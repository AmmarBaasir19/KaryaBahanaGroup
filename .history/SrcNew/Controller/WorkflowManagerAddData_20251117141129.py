        ...
        try : 
            if value1 or value2 : 
                ...
                if ComponentsButton().button_download(): 
                    ...
                    st.write(SessionManager().get_setter_state(key='dataframe_inline').columns)
                    st.write(SessionManager().get_setter_state(key='dataframe_noninline').columns)
                    st.write(f"After Lower Case")
                    st.write(df1_temp.columns)
                    st.write(df2_temp.columns)
                    st.write(df_users_temp.columns)
                    ...
                    st.write(f"After Rename Columns")
                    st.write(df1_temp.columns)
                    st.write(df2_temp.columns)
                    st.write(df_users_temp.columns)
                    ...
                    SessionManager().set_setter_state('dataframe_inline', df1_temp) 
                    SessionManager().set_setter_state('dataframe_noninline', df2_temp)
                    SessionManager().set_setter_state('dataframe_users', df_users_temp)
                    ...
                    df1_temp.to_sql('fd_inline', con=DatabaseConfig().connect_to_db(), if_exists='append', index=False, dtype=DatabaseConfig().mapping_fd_inline()) 
                    df2_temp.to_sql('fd_noninline', con=DatabaseConfig().connect_to_db(), if_exists='append', index=False, dtype=DatabaseConfig().mapping_fd_noninline()) 
                    df_users_temp.to_sql('users_activity', con=DatabaseConfig().connect_to_db(), if_exists='append', index=False, dtype=DatabaseConfig().mapping_users_activity()) 
        ...
        try : 
            if value1:
                ...
                if ComponentsButton().button_download(): 
                    ...
                    df_temp_user = SessionManager().get_setter_state(key='dataframe_users')
                    df_temp.columns = df_temp.columns.str.lower()
                    df_temp_user.columns = df_temp_user.columns.str.lower()
                    ...
                    SessionManager().set_setter_state(key='dataframe_fpd', value=df_temp)
                    SessionManager().set_setter_state(key='dataframe_users', value=df_temp_user)
                    ...
                    SessionManager().get_setter_state(key='dataframe_fpd').to_sql('fpd', con=DatabaseConfig().connect_to_db(), if_exists='append', index=False, dtype=DatabaseConfig().mapping_fpd()) 
                    SessionManager().get_setter_state(key='dataframe_users').to_sql('users_activity', con=DatabaseConfig().connect_to_db(), if_exists='append', index=False, dtype=DatabaseConfig().mapping_users_activity()) 
