...
class ControllerReports:
    ...
    def run(self, df1: pd.DataFrame, df2: pd.DataFrame, start_date: pd.to_datetime, end_date: pd.to_datetime) -> pd.DataFrame:
        ...
        try : 
            ...
            print(f"Ini setelah filter_date (ControllerReports) : {df}")
            df = self.generate_periode(df)
            print(f"Ini setelah generate_periode (ControllerReports) : {df}")
            print(f"Ini setelah generate_periode type : {type(df)}")
            print(f"Ini setelah generate_periode columns: {df.columns}")
