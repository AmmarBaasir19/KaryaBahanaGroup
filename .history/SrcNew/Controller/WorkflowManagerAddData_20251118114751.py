        ...
        try : 
            if value1 or value2 : 
                ...
                if SessionManager().get_setter_state(key='error_page'): 
                    DataProblemScanning().run(input_pdf=SessionManager().get_setter_state(key='pdf_input'), error_page=SessionManager().get_setter_state(key='error_page'), file_name=FileManager().get_format_name(SessionManager().get_setter_state(key='dataframe_fd')), type_form=type_form) 
