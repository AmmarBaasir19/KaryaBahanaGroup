...
class DataPreprocessingReports:
    def __init__(self):
        ...
        self.columns_data2_int = [
            ...
            'First Pass Def', 'Total Top', 'Total Middle', 'Total Bottom'
        ]
    ...
    def change_columns_name(self, df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
        ...
        return df1.rename(str.title, axis=1), df2.rename(str.title, axis=1)
    ...
    def rename_columns(self, df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
        ...
        try : 
            ...
            df2 = df2.rename(columns={'Total_Repair': 'First Pass Def'})
    ...
    def wrangling_data(self, df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
        ...
        try : 
            df2['Total Middle'] = df2['Total Middle'].replace('-', 0)
            return df1, df2
    ...
    def change_data_type(self, df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
        ...
        try : 
            df1[self.columns_data1_int] = df1[self.columns_data1_int].astype('int64', errors='ignore')
            df1[self.columns_data1_str] = df1[self.columns_data1_str].astype(str)
            df2[self.columns_data2_int] = df2[self.columns_data2_int].astype('int64', errors='ignore')
    ...
    def run(self, df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
        ...
        try : 
            ...
            print(f"Ini setelah change_columns_name df1 : {df1})") 
            print(f"Ini setelah change_columns_name df2 : {df2})")

            df1, df2 = self.rename_columns(df1, df2)
            print(f"Ini setelah rename_columns df1 : {df1})")
            print(f"Ini setelah rename_columns df2 : {df1})")
            ...
            print(f"Ini setelah drop_duplicated df1 : {df1})")
            print(f"Ini setelah drop_duplicated df2 : {df1})")

            df1, df2 = self.change_data_type(df1, df2) 
            print(f"Ini setelah change_data_type df1 : {df1})") 
            print(f"Ini setelah change_data_type df2 : {df1})") 
